...
def icp(reference_points, points, max_iterations=1000, distance_threshold=0.3, convergence_translation_threshold=1e-3,
        convergence_rotation_threshold=1e-5, point_pairs_threshold=10, verbose=False):
    """
    ...
    nbrs = NearestNeighbors(n_neighbors=1, algorithm='auto').fit(reference_points)
    ...
    for iter_num in range(max_iterations):
        ...
        for nn_index in range(len(distances)):
            if abs(distances[nn_index][0]) < abs(distance_threshold):
        ...
        if len(closest_point_pairs) < point_pairs_threshold:
            # if verbose:
            print(f'No better solution can be found (very few point pairs) at iteration {iter_num}!')
            break
...
def icp_trans(reference_points, points, max_iterations=1000, distance_threshold=0.3, convergence_translation_threshold=1e-3,
        convergence_rotation_threshold=1e-5, point_pairs_threshold=10, verbose=False):
    """
    An implementation of the Iterative Closest Point algorithm that matches a set of M 2D points to another set
    of N 2D (reference) points.
    ...
    for iter_num in range(max_iterations):
        ...
        rot = np.array([[c, -s],
                        [s, c]])
        aligned_points = np.array(points)
        aligned_points[:, 0] += closest_translation_x
        aligned_points[:, 1] += closest_translation_y
