...
from icp import icp
from load_data import get_vector_separate, get_vector, read_axle_data
...
def display_profiles_overlapping(window_name, org_dict: dict, aligned_dict: dict):
    org_pts = np.concatenate((org_dict['left_down'], org_dict['left_up'], org_dict['right_down'], org_dict['right_up']), axis=0)
    aligned_pts = np.concatenate((aligned_dict['left_down'], aligned_dict['left_up'], aligned_dict['right_down'], aligned_dict['right_up']), axis=0)
    ...
    fig.add_trace(
        go.Scatter(x=org_pts[:, 0], y=org_pts[:, 1], mode='markers', marker=dict(symbol='square', size=2),
                   name='ref flange left down laser'))
    fig.add_trace(
        go.Scatter(x=aligned_pts[:, 0], y=aligned_pts[:, 1], mode='markers', marker=dict(symbol='square', size=2),
                   name='ref flange right down laser'))
    fig.show()
...
def prepare_master_dict(fdict: dict, tdict: dict):
    mdict = {}
    if is_faulty:
        mdict.update({'left_down': tdict['left_down']})
        mdict.update({'left_up': tdict['left_up']})
    else:
        mdict.update({'left_down': np.concatenate((fdict['left_down'], tdict['left_down']), axis=0)})
        mdict.update({'left_up': np.concatenate((fdict['left_up'], tdict['left_up']), axis=0)})

    mdict.update({'right_down': np.concatenate((fdict['right_down'], tdict['right_down']), axis=0)})
    mdict.update({'right_up': np.concatenate((fdict['right_up'], tdict['right_up']), axis=0)})
    return mdict
...
def run_non_linear_optimization(main_folder, axle_id):
    ...
    if load_field_data:
        ...
        if divide_profile:
            f, t = get_vector_separate(main_folder + "\\laser_profile_1.json")
            fmaster_dict['org_points']['left_down'], tmaster_dict['org_points']['left_down'] = f[:, :2], t[:, :2]

            f, t = get_vector_separate(main_folder + "\\laser_profile_2.json")
            fmaster_dict['org_points']['left_up'], tmaster_dict['org_points']['left_up'] = f[:, :2], t[:, :2]

            f, t = get_vector_separate(main_folder + "\\laser_profile_3.json")
            fmaster_dict['org_points']['right_down'], tmaster_dict['org_points']['right_down'] = f[:, :2], t[:, :2]

            f, t = get_vector_separate(main_folder + "\\laser_profile_4.json")
            fmaster_dict['org_points']['right_up'], tmaster_dict['org_points']['right_up'] = f[:, :2], t[:, :2]
    ...
    if divide_profile:
        ...
        if is_faulty:
            ...
            fmaster_dict['aligned_points'][0] = compute_icp('left_down', ['left_up', 'right_down', 'right_up'], fmaster_dict['org_points'],
                                                            [False, False, False])
            fmaster_dict['aligned_points'].append(laser_dict.copy())
            fmaster_dict['aligned_points'][1] = compute_icp('right_down', ['left_down', 'left_up', 'right_up'], fmaster_dict['org_points'],
                                                            [False, False, False])
        ...
        if divide_tread_profile:
            ...
            twall_master_dict['aligned_points'].append(laser_dict.copy())
            tsurf_master_dict['aligned_points'].append(laser_dict.copy())
            tmaster_dict['aligned_points'].append(laser_dict.copy())

            twall_master_dict['aligned_points'][1] = compute_icp('right_down', ['left_down', 'left_up', 'right_up'], twall_master_dict['org_points'],
                                                                 [False, False, False])

            print("\nRunning ICP for tread surface points")
            print("===================================================================================")
            tsurf_master_dict['aligned_points'][1] = compute_icp('right_down', ['left_down', 'left_up', 'right_up'], tsurf_master_dict['org_points'],
                                                                 [False, False, False])

            tmaster_dict['aligned_points'][1] = prepare_master_dict(twall_master_dict['aligned_points'][1],
                                                                    tsurf_master_dict['aligned_points'][1])
        ...
        master_dict['aligned_points'].append(laser_dict.copy())
        if is_faulty:
            master_dict['aligned_points'][1] = prepare_master_dict(fmaster_dict['aligned_points'][0],
                                                                   tmaster_dict['aligned_points'][1])
        else:
            master_dict['aligned_points'][1] = prepare_master_dict(fmaster_dict['aligned_points'][1],
                                                                   tmaster_dict['aligned_points'][1])

        # ***********************************************************************************************************

        if to_display:
            display_profiles_overlapping('overlapping', master_dict['org_points'],
                                         master_dict['aligned_points'][0])
            display_profiles_overlapping('overlapping', master_dict['aligned_points'][0],
                                         master_dict['aligned_points'][1])
...
if __name__ == '__main__':
    ...
    if load_field_data:
        ...
        main_folder = r"D:\Project_Data\profiles_icp\actual\ry0_01"
        run_non_linear_optimization(main_folder, -1)
